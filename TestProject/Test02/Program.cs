...
namespace Test02 {
    ...
    class Program {
        ...
        private static void Exercise06(List<Person> persons) {
            var averageweight = persons.Where(x => x.Height >= 170).Average(x=>x.Weight);
            Console.WriteLine(averageweight+"kg");
        ...
        private static void Exercise07(List<Person> persons) {
            var weight = persons.Where(s => s.Weight <= 70).ToArray();
            foreach (var person in weight)
